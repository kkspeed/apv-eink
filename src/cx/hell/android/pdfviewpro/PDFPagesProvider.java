...
public class PDFPagesProvider extends PagesProvider {
	...
	private static class BitmapCache {
		...
		final double GAMMA = 0.22;
		final double POW = REVERSE / GAMMA;

		int[] gammaA = new int[MAX_SIZE];
		...
		BitmapCache() {
			...
			//for(int i = 0; i < MAX_SIZE; ++i) {
				//gammaA[i] = (int)Math.min(MAX_VALUE_INT,
						//(int)((MAX_VALUE_DBL * Math.pow(i / MAX_VALUE_DBL, POW)) + 0.5));
			//}
		}
		...
		private Bitmap doGamma(Bitmap src) {
			// create output image
			...
			for(int x = 0; x < width; ++x) {
				for(int y = 0; y < height; ++y) {
					...
					A = Color.alpha(pixel);
					// look up gamma
					R = gammaA[Color.red(pixel)];
					G = gammaA[Color.green(pixel)];
					B = gammaA[Color.blue(pixel)];
